...
cfg.MODEL.WEIGHTS = os.path.join(root_path,"models_detectron2","models_lr_classification",
...
for ind,d in enumerate(random.sample(dicts['test'], 15)):
...
try:
    os.mkdir('./test_results_maskrcnn_lr')
...
for p in sorted_[0:600]:
    ...
    v = v.draw_instance_predictions(outputs["instances"].to("cpu"))
    cv2.imwrite(('./test_results_maskrcnn_lr/test_'+str(ind)+'.jpg').format(
                                                    vis_score_threshold,
                                                    learning_rate,
                                                    num_epochs,
                                                    roi_heads_batch_size_per_im,
...
os.makedirs(os.path.join(root_path,
                        'logs',
                        'test_lr_classification'), exist_ok=True)
from detectron2.utils.logger import setup_logger
setup_logger(output = os.path.join(root_path,
                        'logs',
                        'test_lr_classification',
...
from MyTrainer_lr_classes import MyTrainerLR #this loader uses custom datamapper (does not involve RandomFlip)
...
DetectionCheckpointer(model).load(os.path.join(root_path,"models_detectron2","models_lr_classification",
                                 "model_lr{}_ep{}_bspim{}_impb{}_thr{}.pth".
                                    format(
                                        learning_rate,
                                        num_epochs,
...
test_loader = MyTrainerLR.build_test_loader(cfg, "hand_test")
